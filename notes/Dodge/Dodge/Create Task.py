...
class projectile(pygame.sprite.Sprite):
    def __init__(self, damage, velocityh, velocityd, image):
        pygame.sprite.Sprite.__init__(self)
        hitp1 = pygame.sprite.groupcollide(projectiles, p1fighters, False, False)
        hitp2 = pygame.sprite.groupcollide(projectiles, p2fighters, False, False)
        if len(hitp1) > 0:
            for i in p1fighters:
                i.take_damage()
        if len(hitp2) > 0:
...
class SANS(pygame.sprite.Sprite):
    ...
    def Crouch(self):
        ...
        self.image.blit(void,(0,0))
        if self.direction == 0:
            self.image.blit(self.sprites[33],(0,0))
            self.current_image = self.sprites[33]
        else:
            self.image.blit(self.sprites[63],(0,0))
            self.current_image = self.sprites[63]
    ...
    def attck(self):
        self.attacking = True
        if self.current_image != self.sprites[31]:
            self.image.blit(void,(0,0))
        if self.on_stage:
             for event in pygame.event.get():
                 if event.type == pygame.KEYDOWN:
                        ...
                        if event.key == pygame.K_n:
                            if self.direction == 0:
                                 self.current_image = self.sprites[31]
                                 self.image.blit(self.sprites[31],(0,0))



sans = SANS(sans_sprites, "sans", True, False, 264, 156)
extremely_ready = 0
...
while menuing:
    ...
    print(sans.current_image)
    print(len(sans_sprites))
    screen.blit(sans.current_image, [20, 20])
