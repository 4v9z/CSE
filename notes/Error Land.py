...
class Room(object):
    def _init__(self, name='ROOM', description='This is a room', north=None, south=None, east=None, west=None,
...
class Bag(object):
    ...
    def fuse(self):
        if key_1 and key_2 and key_3 and key_4 in self.inventory:
            ...
            Inventor.inventory.remove(key_1)
            key_2.grabbed = False
            Inventory.inventory.remove(key_2)
            key_ory.inventory.remove(key_4)
    ...
    def take_mp(self):
        if player.choice.lower() == "fire blast":
            ...
            if player.MP >= 10:
                print("Thunder is casted on %s and 25 damage is taken" % self.name)
                self.health -= 25
                plahas been defeated!" % self.name)
                    print("%s h += self.money
            else:
                print("You do not have enough MP to cast this")
        elif player.choice.lower() == "blizzard":
            if player.MP >= 15:
                ...
                if self.health < 0:
                    self.health = 0
                    print("%s zard is casted on %s and 35 damage is taken" % self.name)
                player.MP -= 15
                self.health -= 50
                if self.health < 0:
                    self.health = 0
                ...
                if self.health < 0:
                    self.health = 0
                    print("%s %s has %d health left" % (self.name, self.health))
            else:
                print("You don't have enough MP to cast this")
